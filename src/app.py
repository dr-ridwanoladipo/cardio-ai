    ...
    if predict_button:
        ...
        if positions_data and not pos_error:
            ...
            with gcol2:
                ...
                st.markdown(f"**Exercise Capacity:** {guidelines.get('exercise_capacity', 'N/A')}",
                            help="Based on % of age-predicted max HR (220-age)")
                st.markdown('</div>', unsafe_allow_html=True)
        ...
        with st.expander("💡 **Model Insights & Clinical Context**", expanded=False):
            ...
            <div class="insight-box">
                ...
                <p>This model reflects patterns from the Cleveland Heart Disease dataset:</p>
                <ul>
                    <li><strong>Typical Angina Paradox:</strong> Patients with classic chest pain symptoms often showed lower disease rates, likely due to rapid diagnostic workup and exclusion.</li>
                    <li><strong>Exercise Response Patterns:</strong> Some patients with confirmed CAD showed better exercise tolerance, reflecting complex referral patterns.</li>
                ...
                <p><strong>Clinical Recommendation:</strong> Always interpret AI predictions within the full clinical context and current guidelines. This tool supports clinical judgment.</p>
            </div>
            """, unsafe_allow_html=True)
            ...
            if metrics_data:
                ...
                st.write(
                    f"• **High Sensitivity ({metrics_data['sensitivity']:.1%}):** Excellent at detecting disease")
                st.write(
                    f"• **Moderate Specificity ({metrics_data['specificity']:.1%}):** May flag some healthy patients for evaluation")
                st.write(
                    f"• **Positive Predictive Value ({metrics_data['ppv']:.1%}):** When model predicts disease, correct {metrics_data['ppv']:.1%} of the time")
                st.write(
                    f"• **Negative Predictive Value ({metrics_data['npv']:.1%}):** When model predicts no disease, correct {metrics_data['npv']:.1%} of the time")

    # ---------- 💼 FOOTER ----------
    st.markdown("---")

    st.warning(
        "⚠️ This AI tool is for decision support only. "
